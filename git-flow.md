...
프로젝트 규모가 째깐하다면 branch 하나에서 모든 작업을 해도 괜찮을 지 모르지만, 개발의 규모가 커진다면 얘기가 달라진다. 별도의 branch 관리 전략 없이 무작정 개발을 하면 무분별하게 branch를 생성하고 merge하게 되어 막 충돌 나고 이제 난리 난다. git 충돌 해결하느라 삽질 할 시간에 개발을 한다면 생산성이 더욱 올라갈 것이다. 따라서 git flow는 협업 시 효율적인 프로젝트 관리와 생산성을 위해, 궁극적으로는 서비스의 품질을 향상시키기 위해서이다. ~~개발자의 정신건강도~~
